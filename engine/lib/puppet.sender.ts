...
class GCloudAutomation {
  ...
  async createBrowser(headless = true) {
    ...
    // is there need. looks like an override
    //callers can decide whether to run headless or not based on debug flag. If debug is true, we want to force headless to false, otherwise use the passed value.
    if (this.config.debug) {
      //headless = false; //no need for now.
    }

    /**
     * TODO: Test and come back here to check the override of headless mode. If debug is true, we want to force headless to false, otherwise use the passed value.
     * This is to ensure that when debug mode is on, we always see the browser for debugging purposes.
     * If debug mode is off, we can use the headless parameter to determine whether to run in headless mode or not.
     */
    const launchConfig = {
      headless: headless, // test mode first...//headless ? true : false,
 ...
 async manualLogin() {
  ...
  async navigateToAudiencePage(page:puppeteer.Page) {
    ...
    await page.screenshot({
      path: `${this.logDir}/audience-page-loaded.png`,
      fullPage: true,
    });
  }

  async addSingleEmail(page:puppeteer.Page, email:string) {
    console.log(`\n Processing: ${email}`);
    ...
    try {
      ...
      await delay(2000);
      await page.screenshot({ path: `${this.logDir}/panel-opened.png`, fullPage: true });
      ...
      await page.screenshot({ path: `${this.logDir}/email-entered.png`, fullPage: true });
      ...
      await page.screenshot({
        path: `${this.logDir}/after-adding-${email.replace(/[@.]/g, "-")}.png`,
        fullPage: true,
      });
      ...
      await page.screenshot({
        path: `${this.logDir}/error-${email.replace(/[@.]/g, "-")}.png`,
        fullPage: true,
      });
      return false;
    }
  ...
  async clickAddUsersButton(page:puppeteer.Page) {
    const strategies = [
      async () => {
      ...
      async () => {
        ...
        for (const xpath of xpaths) {
            //$.x
          const element = await page.$(`::-p-xpath(${xpath})`);// await page.$x(xpath);
          if (element) {
  ...
  async fillEmailInput(page:puppeteer.Page, email:string) {
    const selectors = [
  ...
  async verifyEmailAdded(page:puppeteer.Page, email:string) {
    try {
      await delay(2000);
  ...
  async addAllEmails() {
    ...
    try {
        ...
        // if there are other errors, we can handle them here or rethrow
        throw error;
    }
    ...
    for (let i = 0; i < this.config.emailsToAdd.length; i++) {
      const email = `${this.config.emailsToAdd[i]}`.trim() || "";
...
export default GCloudAutomation;
